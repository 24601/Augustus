...
- **Arohtea/jev-readout** — JavaScript; license **null** (README MIT); **0★**; README SHA `76a99fe5`; HEAD `03734993`; GitHub size **0** with contents. PRIMARY. structured probability readouts. distribution > argmax. Noul 0.5 midpoint. score is expectation not integer. bare HTTP not SDK. Do not copy `npm` / `.env`.
...
- **yibie/laya-jev-lab** — Python MIT; **0★**; README SHA `57bd1832`; HEAD `30ba64dc`; GitHub size **0** with contents. independent Jev API vs Laya. cascade 0.60 matches 78% at 1.8×. noul facts not judgements. yibie/laya-jev-lab ≠ dairui1/jev-lab ≠ BrendanH18/jev-lab. Do not copy keys.
- **umstek/zero-shot-ie-bench** — Python MIT; **0★**; README SHA `d69dc96a`; HEAD `8770b16b`; size **33**. GLiNER vs GLiFormer vs Laya vs Jev. extractors ≠ decision engines. Laya dict-instructions collapse 58.3%. umstek/zero-shot-ie-bench. Do not copy `.env`.
...
- **GaneshVG18/upgrade-radar** — TypeScript MIT; **0★**; README SHA `b7603c01`; HEAD `1e91eb03`; GitHub size **0** with contents. evidence-linked dependency upgrade. Jev never generates filenames. no_direct_evidence ≠ safe to merge. GaneshVG18/upgrade-radar ≠ LYchoon/paper-radar-jev. Do not copy `npm` / keys.
- **lirantal/discoprint** — TypeScript Apache-2.0; **0★**; README SHA `392efd29`; HEAD `c2a3d47e`; GitHub size **0** with contents. discography theme/mood/complexity. five atomic questions one call. lirantal/discoprint. Do not copy `npx` / `.env`.

Pulse (do not invent): Archer still NOT landed. Hub `archerhume/4rcherhume` HTTP **401**. Tracker likes **56** (flat vs §104 **56**); lastModified `2026-09-20T04:29:16Z` **UNCHANGED** vs §104. Laya Hub HTTP **200** likes **672**. Blackwood Hub HTTP **200** likes **2** gated manual — user census **absent** from the tracker; do not rewrite as landed. Live REST: SemIf **2047★** (flat vs §104 **2047**); jevlike **1018★** (flat vs §104 **1018**); TypeAR-AI/TypeAR **12★** **flat**. AnotiaWang/awesome-jev **91★** (flat vs §104 **91**) ≠ Awesomejev 561/27007. Qwen3.8-27B ≠ Archer. X MCP not used this hour; no invented tweets. `invented_signal: false`.
