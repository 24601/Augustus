...
- **gulagala001/jevify** — JavaScript MIT; **0★**; README SHA `0e2f8536`; HEAD `3d3e904a`; LICENSE SHA `66869720`; GitHub size **0** with contents. Jev-style Choice/Score/Noul from ordinary models. optional DSH plugin. schema-valid ≠ calibrated. gulagala001/jevify ≠ Mintzs/jevify. Do not copy `pnpm` / keys.
- **mourad-ghafiri/laya-rlcd-benchmark** — Python; license **null**; **0★**; README SHA `d8d4859e`; HEAD `3401ff26`; GitHub size **0** with contents. Laya RLCD benchmark. 40.3% below constant-answer. open-weight measurement. mourad-ghafiri/laya-rlcd-benchmark ≠ yibie/laya-jev-lab. Do not copy `uv`.
- **SupremeDreamZ/jev-fastloop** — Python MIT; **0★**; README SHA `053a0885`; HEAD `1157841a`; GitHub size **0** with contents. cheap fail-open semantic edge. second signal not sole. FastLoopError catch. SupremeDreamZ/jev-fastloop ≠ jev-ultrafast. Do not copy keys / Zen UA.
- **TheWebDevel/jev-fanout** — Python MIT; **0★**; README SHA `394b2e1e`; HEAD `b30aaadc`; GitHub size **0** with contents. asking more questions in one call. 0.980 at every N. nearly not fully deterministic. TheWebDevel/jev-fanout. Do not copy `uv` / `.env`.
- **harneet2512/reflexrl** — Python MIT; **0★**; README SHA `e6ff13cd`; HEAD `aa36be84`; GitHub size **0** with contents; default **master**. Qwen3-VL perception + Jev decisions train RL. 0 model calls at deployment. VLM alone 1.7 vs +Jev 4.4. harneet2512/reflexrl ≠ khordoo/jev-reflex-autonomy-lab. Do not dump demos.
...
- **GaneshVG18/upgrade-radar** — TypeScript MIT; **0★**; README SHA `c32f7d18`; HEAD `e438f9bd`; GitHub size **0** with contents. evidence-linked dependency upgrade. Jev never generates filenames. no_direct_evidence ≠ safe to merge. GaneshVG18/upgrade-radar ≠ LYchoon/paper-radar-jev. Do not copy `npm` / keys.
- **lirantal/discoprint** — TypeScript Apache-2.0; **0★**; README SHA `9a64f473`; HEAD `a9d3294f`; GitHub size **0** with contents. discography theme/mood/complexity. five atomic questions one call. lirantal/discoprint. Do not copy `npx` / `.env`.

Pulse (do not invent; independent review relock after `92577f5`): Archer still NOT landed. Hub `archerhume/4rcherhume` HTTP **401**. Tracker likes **56** (flat vs §104 **56**); lastModified `2026-09-20T04:29:16Z` **UNCHANGED** vs §104. Laya Hub HTTP **200** likes **690**. Blackwood Hub HTTP **200** likes **2** gated manual — user census **absent** from the tracker; do not rewrite as landed. Live REST: SemIf **2059★** (+12 vs §104 **2047**); jevlike **1022★** (+4 vs §104 **1018**); TypeAR-AI/TypeAR **12★** **flat**. AnotiaWang/awesome-jev **92★** (+1 vs §104 **91**) ≠ Awesomejev 561/27007 ≠ yibie/awesome-jev **443★**. Qwen3.8-27B ≠ Archer (likes **15794**). X MCP not used this hour; no invented tweets. `invented_signal: false`.
