...
- **ASEVlad/jev-injection-bench** — Python MIT; **0★**; HEAD `c0d0f25d`; README SHA `1b28498c`; LICENSE SHA `0dab34c6`; size **0** WITH CONTENTS. PRIMARY. 11,900 labelled prompts. Jev best ranking / Haiku better ECE 0.021 vs 0.058. 0.5–0.9 band is where Jev's numbers do not mean what they say. Prompt wording moves panic 28%. Do not copy `TYPESAFE_API_KEY` / `ANTHROPIC_API_KEY` / `HF_TOKEN`.
...
- **sshariqali/jev-abstentionbench** — Python; README MIT / GitHub SPDX **NOASSERTION**; **0★**; HEAD `47a4ca0f`; README SHA `26393f0f`; LICENSE SHA `7f6fb3ba`; size **0** WITH CONTENTS. AbstentionBench-on-Jev rank 1 of 20 vs 2025 field. question-asymmetry. forward-looking 0.465 never extreme.
- **misakaikato/openkev** — Python MIT; **0★**; HEAD `864a2207`; README SHA `974d97f9`; LICENSE SHA `8c3231cb`; size **484**. openkev calibration layer not a runtime. ECE vs coverage independent. select_threshold returns inf. escalation catches uncertainty not ignorance. ≠ jaredpalmer/kev.
