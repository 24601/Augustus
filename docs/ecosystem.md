...
Architecture / mental models / Harbor-jevals / toolbelt, not a `uv` / `npx` / `go install` / plugin-marketplace catalog. `notes.md` §93. Skip Archer. Quote READMEs. Soft Noul ≠ hard safety. Do **not** re-fold 1441 / §92. Fresh PR; never reopen merged #7 / **#8** / **#9**; do not push onto #10. 0★ HIGHs still get real cards. rh-guard owns the injection-firewall / CI-gate cousins; Augustus owns placement.
...
- **webNeat/llama-jev** — TS; license null; **0★**; README SHA `5ec204af`; HEAD `fdf989a`; size 8. llama-jev llama.cpp replica. Softmax ≠ Noul. **≠** TypeSafe **≠** pcdServer.
