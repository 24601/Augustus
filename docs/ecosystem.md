...
- **reachjalil/system-one-bench** — JavaScript MIT; **0★**; README SHA `3e7109bf`; HEAD `4a83bae6`; LICENSE SHA `c49c6fba`; GitHub size **0** with contents. PRIMARY. independent System One evidence catalog. scores not one leaderboard. no external record currently reproduced. TokenTrim no-Jev matched hybrid 62.4%. reachjalil/system-one-bench ≠ mallahyari/system-one-benchmark. Do not copy `npm` / keys.
...
- **nafisazizir/hev** — Python Apache-2.0; **0★**; README SHA `b995dce3`; HEAD `79a486f9`; LICENSE SHA `54baf100`; GitHub size **0** with contents. option isolation (sibling-blind). permutation-equivariant. Hub OWNER not published. nafisazizir/hev ≠ jaredpalmer/kev. Do not copy Hub `OWNER`.
- **yuki-oshio/mini-jev** — Python MIT; **0★**; README SHA `363441b6`; HEAD `dff5b323`; LICENSE SHA `79e2cff7`; GitHub size **0** with contents. frozen local LLM logits, no trained decision head. residual-head 9,222-param decreased 73/96→67/96. confidence = 1−normalized entropy, not P(correct). yuki-oshio/mini-jev ≠ r-ms/mini-jev. Do not dump GGUF.
- **erik-dunteman/ChatJev** — Python; license **null**; **0★**; README SHA `c763be19`; HEAD `ea33ab8d`; GitHub size **0** with contents. Jev classifier as autoregressive next-token predictor. ChatJev-style soundness theater. erik-dunteman/ChatJev ≠ dannote/jev ≠ jev-gpt. Do **not** dump `main.py`.
...
- **zzzzzec/jevsort** — HTML; license **null**; **1★**; README SHA `cbe5e6bf`; HEAD `2d960ab9`; GitHub size **0** with contents. parallel rank-prediction vs serial selection. independent questions can conflict. zzzzzec/jevsort ≠ keltokhy/jsort.
- **rupeshpoojary9/awesome-open-system-one** — CC0 1.0 LICENSE SHA `36848c0b`; GitHub SPDX NOASSERTION; **0★**; README SHA `0007e343`; HEAD `637ee3d3`; GitHub size **0** with contents. curated open System One ecosystem catalog. rupeshpoojary9/awesome-open-system-one ≠ AnotiaWang/awesome-jev. Do not paste listed von/4esv numbers.
...
Pulse (do not invent): Archer still NOT landed. Hub `archerhume/4rcherhume` HTTP **401** — not re-fetched as a rewrite. Tracker likes **54**; lastModified `2026-09-20T02:59:13Z`. Live REST (review relock): SemIf **2012★** (+12 vs §102 **2000**); jevlike **1006★** (+4 vs §102 **1002**); TypeAR-AI/TypeAR **12★** **flat**. AnotiaWang/awesome-jev **86★** **flat** ≠ Awesomejev 561/27007. X MCP not used this hour; no invented tweets. `invented_signal: false`.
