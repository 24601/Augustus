...
- **Meanblock/JEV-CPU** — HF mit; likes **6**; sha `759fa606`. GH Meanblock 404; lock leesk212/JEV-CPU (**2★**, size **14336**). softmax over letter slots ≠ Noul. Do **not** treat Hub Meanblock as a new species vs leesk212.
- **impacte/mimir-lfm-openjev** — HF LFM Open License; likes **0**; sha `2b999b3a`. WANLI 0.741 vs openjev v2 0.77 *theirs*. 3-way NLI ≠ Noul.
...
- **Running-Dolphins/jev-bench** — Python MIT; **0★**; HEAD `67d2ee42`; README SHA `ae2c5003`; LICENSE SHA `891c70c6`; size **0** WITH CONTENTS. PRIMARY. “0.9 is not one number”. ranking ≠ calibration. banking77 0.8–0.9 stated 0.86 actual 0.73 over-confident *theirs*. ≠ Praveenrajus/jev-bench ≠ fstandhartinger/jevbench.
...
- **RadRebelSam/jev-decision-lab** — TypeScript MIT; **0★**; HEAD `e6d6d42d`; README SHA `52a0071d`; LICENSE SHA `ae6f994c`; size **0** WITH CONTENTS. Function-only 5/8 vs hybrid 8/8. 4/8 without Jev. 8 designed cases not conversion lift. ≠ RadRebelSam/awesome-jev.
- **jackojacko05/compare-jev-bigquery-ai-functions** — Jupyter MIT; **0★**; HEAD `fca3771a`; README SHA `37dc973e`; LICENSE SHA `0248c4b0`; size **0** WITH CONTENTS. 200-row pilot Jev 85.5% 171/200 vs Gemini Flash-Lite 86.0% 172/200 vs Pro 88.5% 177/200 *theirs*. not a ranking.
...
- **joshhu/jevtest** — HTML; license **null**; **0★**; HEAD `6a4df41d`; README SHA `57d99325`; size **0** WITH CONTENTS. 情緒測謊器. 1q 396ms / 30q 567ms. ±0.03. 33q $0.000045 vs Gemini ~5× slower ~60× cost *theirs*. ≠ realZachi/jevtest.
...
- **AiPersonacademy/Awesome-jev-use** — CC0-1.0; LICENSE SHA `7ba0a23e`; **2★**; HEAD `7cd1064c`; README SHA `32b0ddf5`; size **124**. APA “microsecond policy / zero hallucination” overclaim. Catalog ≠ endorsement.
...
- **makefunstuff/jev-lsp** — Rust MIT; **0★**; HEAD `734c0f59`; README SHA `c7ecbf8f`; LICENSE SHA `fb5bc4a5`; size **1696**. pattern exact, judgement must clear floor. no matching pattern → no model call. not a correctness oracle. $0.00022 vs chat $0.00306 *theirs*.
- **nozomi-koborinai/jev-spec** — TypeScript MIT; **1★**; HEAD `9f8cece7`; README SHA `7c5b08d0`; LICENSE SHA `e43f732a`; size **73**. Spec vs artifact remainder. treating 0.85 as 85% / minProbability hard-gate as Harbor.
