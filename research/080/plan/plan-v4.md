...
| P5 | Episode-level selection beats proxy selection | **[Rep] MEASURED BY E3, 2026-09-25, and NOT supported at the registered margin.** Outcome-selected minus proxy-selected is +0.0375 utility on 6,405 held-out questions, interval [+0.0163, +0.0588], against a 0.02 margin: powered, right direction, LCB below the margin. Search said +0.060, so selection on the search split cost 37% of the effect and that was enough to move the claim. The direction survives and the size does not | E3, **done** |
| P6 | Untouched incumbent–challenger lowers false adoption | [Rep sim] −1.8 vs −0.26 pp. **E3 2026-09-25: not supported in that setting** — 0.000 against 0.005 over 200 resplits, difference −0.005 with 95% CI [−0.0148, +0.0048] crossing zero. With an incumbent that is genuinely near-best there is too little false adoption to measure a reduction in. E4a's planted-defect evidence is unaffected | E3 **done**, E4a |
...
| P11 | Programs lose on fuzzy real text | **[H]**, stated in advance so the M5 result is not read as a defect. RAP's synthetic contrast pairs cannot establish it (Astra v4 F10). **Split on the evidence, 2026-09-25**: confirmed on T2a, where A1 costs 0.4058 against R2a's 0.1177; refuted on T2c, where A1's written lexicon costs 0.0587 and beats three of the four model rungs. A prediction that holds on a 77-way decision and fails on a binary one is about how much of the decision the label names carry, not about programs. A2b is outstanding; see `receipts/m5-grading-2026-09-25.md` | M5's T2a–c |
...
| Tasks | **T1** (generated, code-labeled): `fixture_evidence_only`. **Real-text confirmation**, human-written, seeded equal-probability splits, the population being the benchmark and not deployment traffic: **T2a** BANKING77, 77-way, CC BY 4.0, about 13.1k rows, real-query provenance checked at M2; **T2b** CLINC150 plus OOS, route-or-abstain, reusing E1's seeded CLINC partition; **T2c** CivilComments, the E1-disjoint 100k pool. Fixed cost matrices: misroute 1, abstain 0.3, correct 0 for T2a/T2b; C_FP:C_FN = 1:4 for T2c. Losses ∈ [0, 1], so paired differences have range R = 2. **Contamination gate, added 2026-09-24** (`sources/bev-decision-150k-2026-09-24.md`): the public `avbiswas/bev-decision-150K` training mixture is built partly from CivilComments, CLINC150 and BANKING77, which are exactly T2a–c. A candidate whose training corpora include, or plausibly include, that mixture cannot have a T2 result read as held-out; it declares its corpora and is checked, or the result is reported as contaminated. **First named instance, 2026-09-25** (`sources/decider-2026-09-25.md`): `Mapika/decider` trains on `mteb/banking77`, `clinc/clinc_oos` config plus, and `google/civil_comments`, read from `decider/data/core.py` rather than from its README, and its `heldout=True` flag is applied to 24 other tasks but not to these three. It is therefore barred from T2a, T2b and T2c as a held-out comparator despite Apache-2.0 weights and a public recipe. HotpotQA is absent from that mixture, so E3's corpus is unaffected. Seeded partitions give no protection, because the leak is upstream of the partitioner |
...
| M5 | Trainer reproduction: R0, A1, A2a, A2b, R1, R2a, R2b vs R3a (R3b if parity passes). **Partitioned 2026-09-25; BLOCKED on acquisition** (`receipts/m5-envelope-2026-09-25.md`): inside the pinned image only A1 and R2a of the six family members are reachable, and the R3a comparator is not, because `setfit` and `paw` are absent and Qwen3.5-2B-Base is not staged. The accelerator constrains nothing. Since every outcome row is a contrast against R3a, an unreachable comparator makes all of them unreadable, so the §3.3 rule already gives the answer: **Inconclusive, no rung recommendation**. **Maintainer authorized a newly pinned image, 2026-09-25.** Unblocking needs a newly pinned image carrying `setfit` and the PAW compiler, plus bounded downloads of Qwen3.5-2B-Base and the PAW checkpoints — not an edit to the current image and not a reduced family. **Known risk, from `sources/paw-rap-2026-09-23.md`: the PAW compiler states NVIDIA and BF16 and says ROCm is untested**, and tabputer is gfx1151, so A2a and A2b may remain unreachable after the image exists. That is a separate finding from a missing package and is to be reported as one | M1, M3; Qwen3.5 and PAW parity | 3–4 | §3.3 rule applied, including inconclusive |
...
| M6 | Author `augustus-train`; hand-off; scenarios; independent review; activation suite. **Not blocked by M5's outcome**: §3.3's inconclusive row says the default stays the procedure, so the skill ships the gated ladder and the envelope declaration of §3.7 either way, and states that the 0.8.0 reproduction was inconclusive at 1 pp | M5 | 2–3 | Budgets pass; no unresolved severe flaw |
