...
## Final confirmation round (Astra-requested lane, 2026-09-24, HEAD `3d0463a`)

Verdict: **NOT ACCEPTED — 0 P0, 2 P1, 2 P2.** R3, R4, R5 and R6 are **closed**; R1 and R2 are
downgraded to P2 residues; two new P1s are found in the fixes themselves. All four are accepted
and fixed here. `calc_v4.py` re-ran byte-identical at that HEAD. After these fixes the digests are `b390eb15…` (script) and `d6f9b745…` (output).

| ID | Sev | Residue or new defect | Disposition | Fix |
|---|---|---|---|---|
| R1 residue | P2 | The plan advertised `−margin − true_delta` for superiority, but `calc_v4.py` implemented `|true_delta| − margin`. The absolute value hid a direction error: M5's escalation row has the **incumbent** better, so it is an LCB read, not a UCB read | **Accepted** | Both §2.7 and `calc_v4.py` now carry **two superiority modes**: `sup_ucb` (candidate better, `−margin − true_delta`) and `sup_lcb` (incumbent better, `true_delta − margin`). The M5 escalation row is labeled `sup_lcb`, and the script asserts a positive distance, so a wrong direction is a specification error rather than a plausible-looking n |
