...
- `.agents/skills/augustus/references/applied-mappings.md` — context sieve,
  ...
  hourly 1541 (`notes.md` §93; difficulty + policy thresholds + JSONL trace; jev-codex-pilot model + reasoning depth; keep/shadow/hybrid/reject; quarry evidence projection; Frank-ZY-Dou/awesome-jev robotics/3D/control; one-dollar-tahoe TypeSafe Jev defense eval; jevguard calibrator/cache/escape; jev-ci-selector CI shadow mode; llama-jev llama.cpp replica; rh-guard owns gates; Soft Noul ≠ hard safety)
...
- `.agents/skills/augustus/references/validation.md` — design gate, eval
  ...
  *theirs*;
  ...
  hourly 1541 (`notes.md` §93; orchestrator finish 0.95 / test 0.8 *theirs* sensors; replacement 20-case acc 0.90→1.00 F1 0.9028→1.0000 p50 675.66→253.13 ms cost 1.52× *theirs* failed cost gate; quarry 5 s fail-open / p&lt;0.5 drop / top 3; OpenRoboto apple-to-plate 113 cycles $0.018825 vs Astra $5.933624 *theirs* one seed-0 not a rate; one-dollar-tahoe ~74 demo no ASR/FPR; llama-jev 80 ms cold / 40 ms cache *theirs* softmax ≠ Noul))
