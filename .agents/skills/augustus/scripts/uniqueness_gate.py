...
UNIQ_2044 = (
"Hourly 2044 uniqueness lock: TypeSafe Jev default path; MiaoWuNYA/rikkahub-sillytavern-android README SHA f7de16872c8c; HEAD 839eebce673b; release v2.5.4fix13; Jev judges memory and answers the judge tool; the chat model writes; relevant probability over half is an application cut; 0.5 is not a decision boundary; silent fallback when Jev is down; 1104480426-hash/jev-qq-assist README SHA 55f96440225a; release v1.5.0; screen is a sensor; no send path; local bge-small-zh is not TypeSafe; softmax over similarity is not a calibrated Noul; 49-64 ms local versus about 870 ms remote *theirs*; 1104480426-hash/jev-qq-assist \u2260 jev-chat/jev-chat-jarvis \u2260 GxZzzzz/WeChat_Galgame; Iron-LYK/OmniJev README SHA b41caf70e3af; Nemotron generation API; Vision-Jev RLCD coming soon; skill mode is privileged simulator state; not end-to-end vision; softmax over option letters is not calibrated confidence; 13 calls and 39 tokens *theirs*; Nemotron \u2260 Archer; simulation success \u2260 calibrated Noul; bokuweb/omg README title grande; README SHA 3603f12be97d; release ruri-v1; one forward pass no text generation; Gemma 4 E2B JNLI ECE 0.252 to 0.088 *theirs*; 41% of p>=0.9 wrong before scaling *theirs*; temperature scaling is not RLCD; Gemma 4 \u2260 Archer; trained Gemma 4 weights are not landed; serving substrate \u2260 calibrated replica; Vankleben/jev-arm-lab README SHA a48080d4c81e; code owns the veto; freshness gate is code; 20/20 and Brier 0.030 and 0 dangerous false positives *theirs*; wrong protocol scores 77% *theirs*; gates 0.30 and 0.45 still soft until you measure; Waxmell114514/jev-trade paper trading against observed prices; not order placement; this card is not a trading procedure; Waxmell114514/jev-trade \u2260 pgarvie/jev-trade \u2260 klren0312/jev-trade \u2260 jarrodwatts/jev-trader; antTing/jev-accounts-hub a gateway is not a decision model; do not copy the proxy; dhruvmehra/jevbench n=500 *theirs*; banking77 Jev 76.4 ECE 0.125 Laya 38.2 ECE 0.511 *theirs*; AG News Jev 84.3 ECE 0.112 *theirs*; SST-2 Jev 95.4 ECE 0.026 *theirs*; CI about \u00b12.5 points *theirs*; fine-tuned BERT is not a zero-shot peer; dhruvmehra/jevbench \u2260 fstandhartinger/jevbench; accuracy \u2260 calibration; xinyao27/jevonian release v0.0.1; named routes skip Jev; no brain returns an error; 502 when every brain is down; minConfidence does not failover; routing \u2260 permission; 0x440-1me/laya-unity not affiliated; absent candidates cannot be chosen; transport client \u2260 calibrated replica; hf:Cahol/laya-banking77-v1 sha 09a7f1a08aae; 85.55% on 3,080 *theirs*; tuned ECE 0.0237 to 0.0144 *theirs*; no Jev outputs used in training; do not paste over the zero-shot 79.90 row; Laya \u2260 TypeSafe; openlayer-ai/jevals densify \u00a7126; HEAD 0457836c5696 was a38a971095c4; description_hash 139fb51429c7 was 6d6d99246390; local Kev or Laya is their new sentence; 92x to 913x stays the prior citation; one example trace is not a bench; FluidInference/FluidUse densify \u00a7143; HEAD unchanged e9e95935075b; README SHA unchanged 7d246df7270a; description_hash 2e5b91ad5dfb was ddacb829949a; description rewrite is not a new bench; do not re-fold 3.6 ms; Cua-S1 \u2260 TypeSafe; danielyedaniel/jevme densify \u00a7156; HEAD c6c4c4415776 was 110bf6ba7ad3; about 200 ms *theirs*; SHA move is not a replica; TMHSDigital/plumbline densify \u00a7158; HEAD da7b7364d91b; release v0.1.0; not a leaderboard; API will change; aiwithenoch/Jev-Skill densify \u00a7113; HEAD 32a1d5fe5251 was f0cf6d6e; release v1.0.0; a harness is not a model; thisisandreeeee/jev-benchmarks densify \u00a7114; HEAD eaed9dd0cfd6 was e1186b74; BANKING77 Jev 79.90 stays; SPACE-2 94.77 is not BERT-Base 93.02; ECE still on the backlog; hf:liskasYR/laya is not a new measurement; hf:nampham1106/laya-flash 28-byte license; hf:KGFCode/laya-decision-lab HTTP 401; empty SHA HTTP 409; Soft judgment never sole veto; ranking \u2260 calibration; catalog \u2260 endorsement; *theirs* not Harbor; Skip Archer; invented_signal: false; Parent merges only after ADV_PASS; hourly 1949 \u00a7160 are on main; this fold is \u00a7161 only; composition 949-964; findings batch #141; do not reopen or amend PR #23 through #86; does not bump 0.5.0 or 0.5.1; notes.md \u00a7161"
)
...
def main() -> int:
    ...
    if "## 155. User-provided rawwerks/one-system HIGH" not in notes:
        failed.append("notes.md missing §155 heading")
    if notes.count(UNIQ_ONESYSTEM) != 1:
        failed.append(
            f"notes.md one-system lock count {notes.count(UNIQ_ONESYSTEM)} != 1"
        )
    i155 = notes.find("## 155. User-provided rawwerks/one-system HIGH")
    if not (i154 >= 0 and i155 > i154):
        failed.append("notes.md must stay contiguous §154 then §155")
    notes155 = notes[i155:] if i155 >= 0 else ""
    if "this fold is §155 only" not in notes155:
        failed.append("notes.md §155 missing §155-only occupancy")
    if "hourly 1556 §154 are on main" not in notes155:
        failed.append("notes.md §155 missing merged §154 occupancy")
    if "do not push onto open #77" in notes155 or "open #77 owns" in notes155:
        failed.append("notes.md §155 claims open #77")
    if UNIQ_ONESYSTEM not in notes155:
        failed.append("one-system lock missing inside notes.md §155")
    onesystem_frags = (
        "Privacy Demo is not a privacy filter",
        "skips model-based selection",
        "0.5 is a soft example",
        ...
        "routing selection is not permission",
        "Soft judgment never sole veto",
        "gateway is not a calibrated replica",
        "serving port is not a calibrated replica",
        "catalog ≠ endorsement",
        "TypeSafe Jev remains the default recommended path",
        "not equal in adoption",
        "invented_signal: false",
        "Skip Archer",
        "does not bump 0.5.0 or 0.5.1",
        "composition 853-868",
        "findings batch #135",
        "this fold is §155 only",
        "#80",
        "HEAD 1104500a0197737d25334472a42a5a790098105c",
        "description_hash 4576d70e542f",
        "stargazers_count 1",
        "v0.2.0",
        "README SHA fd20bacaade7",
    )
    for frag in onesystem_frags:
        if frag not in UNIQ_ONESYSTEM:
            failed.append(f"UNIQ_ONESYSTEM missing {frag!r}")
    readme_onesystem = (ROOT / "README.md").read_text(encoding="utf-8")
    if UNIQ_ONESYSTEM in readme_onesystem:
        failed.append("README.md must not hold the one-system uniqueness lock")
    for rel in OVERLAYS:
        if rel == "research/notes.md":
            continue
        overlay_onesystem = (ROOT / rel).read_text(encoding="utf-8")
        if UNIQ_ONESYSTEM in overlay_onesystem:
            failed.append(
    ...
    if "## 157. Hourly 1751 HIGH" not in notes:
        failed.append("notes.md missing §157 heading")
    if notes.count(UNIQ_1751) != 1:
        failed.append(
            f"notes.md 1751 lock count {notes.count(UNIQ_1751)} != 1"
        )
    i157 = notes.find("## 157. Hourly 1751 HIGH")
    if not (i156 >= 0 and i157 > i156):
        failed.append("notes.md must stay contiguous §156 then §157")
    notes157 = notes[i157:] if i157 >= 0 else ""
    if "this fold is §157 only" not in notes157:
        failed.append("notes.md §157 missing §157-only occupancy")
    if "hourly 1653 §156 are on main" not in notes157:
        failed.append("notes.md §157 missing merged §156 occupancy")
    if UNIQ_1751 not in notes157:
        failed.append("1751 lock missing inside notes.md §157")
    for frag in (
        "TypeSafe Jev default path",
        "same GitHub id 1379166549",
        "densify \u00a7151 not a sibling first sighting",
        "HEAD e172f3875783 was 39612ea0dbad",
        "README SHA ada0d085a7da was 13000620bb24",
        "release v0.0.2",
        "pyproject 0.0.2",
        "README status line still says v0.0.1",
        "description_hash ce96a63ca053 unchanged",
        "star 1 to 8 is star-noise beside the rename",
        "L0 is not calibration",
        "decision.level raw L0 L1",
        "not affiliated with TypeSafe",
        "banking20 raw flip 0.227 to 0.077 stays the \u00a7151 table",
        "do not re-fold 0.807 and L1 ECE 0.100 as a new result",
        "Jev 1.13.0 acc 0.727 published not rerun",
        "do not headline the maze",
        "Qwen3 \u2260 Archer",
        "Qwen2.5 \u2260 Archer",
        "153 ms median 24/24 *theirs* not Harbor",
        "min-conf 0.75 still soft",
        "irreversible click stops",
        "\u00b10.05 is their margin sentence",
        "$0.0002 and $0.00004 are two author sentences",
        "do not average them",
        "HAR5HA-7663/hunch \u2260 Kelbie/hunch",
        "code verifies the click",
        "confidence below 0.7 escalates",
        "0.7 still soft",
        "sequences verified against the reference is their claim",
        "wire-compat \u2260 logit-equiv",
        "Jev answers bounded questions",
        "apply refuses a changed tree",
        "Jev is not the writer",
        "trained acc 0.6574 ECE 0.1311",
        "temp-scaled ECE 0.0374",
        "Gemma 3 \u2260 Archer",
        "labels are gpt-oss-20b not human",
        "25,002 questions split on state_id",
        "Hub weights stay \u00a7156",
        "acc 0.789 ECE 0.232 versus Jev ECE 0.144",
        "accuracy is not calibration",
        "temperature 0.1006",
        "clamp [0.5, 5.0]",
        "0.24 republished as 0.99",
        "serving substrate \u2260 calibrated replica",
        "base is Laya not TypeSafe",
        "cedrecs/jev-stories densify \u00a7153",
        "HEAD 19b766f7b765 was 34fbe3a8478f",
        "picks never change the outcome",
        "game success \u2260 calibrated Noul",
        "lee-lou2/jev-tree densify \u00a7127",
        "HEAD ddc25cf03913 was 4fd272a26656",
        ...
        "AUC 0.891 / 0.992 / 0.787 *theirs*",
        "a ranker is not a detector",
        "$0.97 is their study cost",
        "101/101 is ceiling-effect evidence",
        "protocol is a draft",
        "confidence_kind none means no probability to threshold",
        "BM25 33/40 versus 28/40 *theirs*",
        "NakliTechie/verdict \u2260 Heman10x-NGU/openJev-verdict-2.0",
        "~340 ms over 1,064 skills *theirs*",
        "Noul per skill",
        "abgregs/jev-skill-router \u2260 aleksvega/jev-skill-router",
        "loom 0.85 still soft",
        "selection does not execute",
        "shadow discards the answer",
        "active needs a passing holdout",
        "the provider call is never skipped",
        "no benchmark numbers yet",
        "82 of 108",
        "boss never killed",
        "Jev is not a calculator",
        "heuristic drives",
        "798 lines over 2000 pieces *theirs*",
        "Laya votes and does not drive",
        "temperature-1 logprobs",
        "DiffusionGemma \u2260 Archer",
        "mmastrac/djev \u2260 Hangzhi/diffusion-jev-sglang",
        "empty SHA HTTP 409",
        "perfume shop name collision",
        "rahiseko-alt/Jev-write is a Claude template",
        "kurousa/jev README is the empty blob",
        "jev-ids/jev-ids.github.io \u2260 jev-ids/jev-ids",
        "TJurijs/jev_sandbox 0.5 still soft",
        "TJurijs/jev_sandbox \u2260 baldpanda/jev-sandbox",
        "slatinwine/jev-cua \u2260 ronadin2002/jev-cua",
        "lifeporterlab/jev-verdict \u2260 NakliTechie/verdict",
        "Soft judgment never sole veto",
        "ranking \u2260 calibration",
        "catalog \u2260 endorsement",
        "SHA move is not a replica",
        ...
        "invented_signal: false",
        "Parent merges only after ADV_PASS",
        "hourly 1653 \u00a7156 are on main",
        "this fold is \u00a7157 only",
        "composition 885-900",
        "findings batch #137",
        "#82",
        "does not bump 0.5.0 or 0.5.1",
        ...
        if frag not in UNIQ_1751:
            failed.append(f"UNIQ_1751 missing {frag!r}")
    readme_1751 = (ROOT / "README.md").read_text(encoding="utf-8")
    if UNIQ_1751 in readme_1751:
        failed.append("README.md must not hold the 1751 uniqueness lock")
    for rel in OVERLAYS:
        if rel == "research/notes.md":
            continue
        overlay_1751 = (ROOT / rel).read_text(encoding="utf-8")
        if UNIQ_1751 in overlay_1751:
            failed.append(
                f"1751 lock must stay in notes.md and the gate fixture, not {rel}"
            )
    for remainder_id in (
        "nokia-applied-research/AnyJev",
        "Gui-daSAlmeida/Layali-Al-Ahlam-Importados",
        "dannote/jev_nx",
        "ninjasweb/jev-mute-words",
        "tenuo-ai/safe-upgrade",
        "14-TR/jev-empirical",
        "AkhilBod/Tidy",
        "AkhilBod/TidyJev",
        "Akicou/system-one-270m",
        "AlexanderJiazx/AutoJev",
        "Bald0Wang/jev-playground",
        "HAR5HA-7663/hunch",
        "JamesANZ/ChatJev",
        "JamesANZ/jev-chrome-blocker-extension",
        "Miyamura80/among-us-jev",
        "NakliTechie/verdict",
        "NourEldinShobier/web-search",
        "Roasbeef/loom-skill-selector",
        "Stephonomon/scribe-verify",
        "TJurijs/jev_sandbox",
        "Taf0711/system-one-compiler",
        "Tatendaz/model-picker",
        "WorthingtonCloud/norsestar-air",
        "Y91R/jev-box",
        "Yutaan5/jev-decision-tool",
        "abgregs/jev-skill-router",
        "anonymze/jev",
        "bvolpato/waya",
        "cbroker1/jev-un-squadron",
        "chenhg5/jev-3d-world",
        "cohenom/laya-2048",
        "cohenom/laya-flappy",
        "cohenom/laya-tetris",
        "ehab-ayman-gharib/border-protocol",
        "hf:MissingPackage/jev-d-0.4b-onnx",
        "hf:Silversys/laya-eurusd",
        "hf:agk4444/laya-typed-decisions",
        "hf:kaivoss/system-one-270m-data",
        "j7708git/jev-tradingview-signal",
        "james947/codex-jev",
        "jeffonelson/jev-bigquery-cloudrun",
        "jev-ids/jev-ids.github.io",
        "jostoz/sweetlips",
        "kurousa/jev",
        "leonininder/remember-me",
        "lifeporterlab/jev-verdict",
        "mahajanparth/JEV_SMARTROBOTCONTROL",
        "mmastrac/djev",
        "mouadse/jev-vs-laya",
        "mrkpatchaa/github-star-classifier-jev",
        "nikkoxgonzales/elmridge-ai-town",
        "rahiseko-alt/Jev-write",
        "rythmn1111/doom-war",
        "silvaan/nav-jev",
        "sksq96/jevgram",
        "slatinwine/jev-cua",
        "soleimanian/jev-accounting-research",
        "cedrecs/jev-stories",
        "lee-lou2/jev-tree"
    ):
        if remainder_id not in notes157:
            failed.append(f"notes.md §157 missing card {remainder_id}")

    if "## 158. Hourly 1851 HIGH" not in notes:
        failed.append("notes.md missing §158 heading")
    if notes.count(UNIQ_1851) != 1:
        failed.append(
            f"notes.md 1851 lock count {notes.count(UNIQ_1851)} != 1"
        )
    i158 = notes.find("## 158. Hourly 1851 HIGH")
    if not (i157 >= 0 and i158 > i157):
        failed.append("notes.md must stay contiguous §157 then §158")
    notes158 = notes[i158:] if i158 >= 0 else ""
    if "this fold is §158 only" not in notes158:
        failed.append("notes.md §158 missing §158-only occupancy")
    if "hourly 1751 §157 are on main" not in notes158:
        failed.append("notes.md §158 missing merged §157 occupancy")
    if UNIQ_1851 not in notes158:
        failed.append("1851 lock missing inside notes.md §158")
    for frag in (
        "TypeSafe Jev default path",
        "IAMIbrahimmemon/von-mlx README SHA aa166f204e7b",
        "do not restate n=78",
        "serving substrate \u2260 calibrated replica",
        "IAMIbrahimmemon/von-mlx \u2260 wfzyx/von",
        "fooSynaptic/jev-any-llm README SHA f7c3b097e06a",
        "15-88x vs free-text on AG News *theirs*",
        "logprobs \u2260 calibrated Noul",
        "early-exit head is not a trained RLCD replica",
        "maskedband1t/RLCD README SHA 60706479ba04",
        "robot success \u2260 calibrated Noul",
        "0.5 is not a decision boundary",
        "AUC 1.000 and 15 of 20",
        "majority class probability 1.0000",
        "urgency 0.733 to 0.648",
        "one seed",
        "held-out sources 0.769/0.107 vs Jev 0.835/0.090 *theirs*",
        "\u00b10.05 across seeds",
        "wire-compat \u2260 logit-equiv",
        "Qwen3.5 \u2260 Archer",
        "republished card is not a new measurement",
        "Khmer 0.000 at 0.952",
        "neilbauman21-hub/verdict \u2260 NakliTechie/verdict",
        "not a leaderboard",
        "do not import a winner",
        "jaredpalmer/kev densify \u00a745 not a sibling first sighting",
        "HEAD 90990a5fac2995b9faa3190f7d437e84f2067768",
        "watch prior prefix 35566d73bea1",
        "stored look 4f8110a3f8620cc3a182ae9a708e4398492c4b1a",
        "no new benchmark result",
        "description_hash 43bc19349316 was 10755cc3be89",
        "empty tree",
        "nadeemcite/jev-crash-course densify \u00a7150",
        "nadyth/jev-crash-course densify \u00a7150",
        "11-level description plus LiteLLM",
        "pedagogy not a bench",
        "routing \u2260 permission",
        "game success \u2260 calibrated Noul",
        "ranking \u2260 calibration",
        ...
        "invented_signal: false",
        "Parent merges only after ADV_PASS",
        "hourly 1751 \u00a7157 are on main",
        "this fold is \u00a7158 only",
        "composition 901-916",
        "findings batch #138",
        "#83",
        "does not bump 0.5.0 or 0.5.1",
        "notes.md \u00a7158",
        "one tracker like is star-noise",
        "Qwen/Qwen3.8-27B \u2260 Archer",
    ):
        if frag not in UNIQ_1851:
            failed.append(f"UNIQ_1851 missing {frag!r}")
    readme_1851 = (ROOT / "README.md").read_text(encoding="utf-8")
    if UNIQ_1851 in readme_1851:
        failed.append("README.md must not hold the 1851 uniqueness lock")
    if UNIQ_JIMOTHY in readme_1851:
        failed.append("README.md must not hold the jimothy uniqueness lock")
    for rel in OVERLAYS:
        if rel == "research/notes.md":
            continue
        overlay_1851 = (ROOT / rel).read_text(encoding="utf-8")
        if UNIQ_1851 in overlay_1851:
            failed.append(
                f"1851 lock must stay in notes.md and the gate fixture, not {rel}"
            )
    for remainder_id in (
        "gholtzap/jev-codex-model-and-effort-router",
        "AditiSonawane96/jev-model-test",
        ...
        "JohnCari/rossrecall",
        "LBDavid98/jevidence",
        "NeerajMohanty/RFxCheck",
        "Roasbeef/jevlar",
        "TMHSDigital/plumbline",
        "Zyw052/astrbot_plugin_jev_radar",
        "baize7815/jev-mcp-open-source",
        "c85/jev-smb",
        "cyberspace-cs/jev-agent-routing",
        "dagote/JevBlock",
        "dpshde/bible-jev-climb",
        "dxd-dechao/jev-playground",
        "fooSynaptic/jev-any-llm",
        "frankda/jev-poly-crypto-demo",
        "galgoczy/JEVsniper",
        "gdamiani1/jev-feed-triage",
        ...
        "tonghzhang/jevproof",
        "wade56754/keyframe-to-motion",
        "yoichiojima-2/deslop",
        "zhijianzhouml/CELEUS-JEV",
        ...
        if remainder_id not in notes158:
            failed.append(f"notes.md §158 missing card {remainder_id}")

    if "## 159. User-provided jimothy" not in notes:
        failed.append("notes.md missing §159 heading")
    if notes.count(UNIQ_JIMOTHY) != 1:
        failed.append(
            f"notes.md jimothy lock count {notes.count(UNIQ_JIMOTHY)} != 1"
        )
    i159 = notes.find("## 159. User-provided jimothy")
    if not (i158 >= 0 and i159 > i158):
        failed.append("notes.md must stay contiguous §158 then §159")
    notes159 = notes[i159:] if i159 >= 0 else ""
    if "this fold is §159 only" not in notes159:
        failed.append("notes.md §159 missing §159-only occupancy")
    if "hourly 1851 §158 are on main" not in notes159:
        failed.append("notes.md §159 missing merged §158 occupancy")
    if UNIQ_JIMOTHY not in notes159:
        failed.append("jimothy lock missing inside notes.md §159")
    for frag in (
        'TypeSafe Jev default path',
        'AndrewPrifer/jimothy distill local-serving path not a replacement',
        'distilled local classifier ≠ TypeSafe Jev',
        '≠ calibrated replica of hosted Jev',
        'npm jimothy 0.1.2',
        'engines node >=22',
        'package.json description Train portable local classifiers from Jev-compatible datasets.',
        'npm registry 0.1.2 matches package.json',
        'GitHub description null',
        'description_hash null',
        'null GitHub description is not an empty README',
        'README SHA 124318338a4c21ddd256df4501f387cc11857e1f',
        'LICENSE SHA 81712734e28d7bcc4421ea1c57654591830af8c3',
        'GitHub id 1378408007',
        'HEAD f2ad9b40b88ea913d38fda758e564eac5f12fc0b',
        'commit 2026-09-21T02:37:20Z',
        'pushed_at 2026-09-21T02:37:21Z',
        'updated_at 2026-09-22T01:09:47Z is not a new HEAD',
        'release_tag null',
        'teacher typesafe-ai/jev when configured',
        'AI_GATEWAY_API_KEY',
        'training itself is local',
        'a supplied teacher label is a reference for agreement not independent ground truth',
        "calibrated outputs may differ from the teacher's original probabilities",
        'do not distill Jev as teacher of record',
        'real outcomes stay the targets',
        'null threshold means no recommendation',
        'insufficient_data or target_not_met',
        'small quick-start',
        'email 300 threshold None',
        'SDK returns every prediction',
        "applying a cutoff is the application's choice",
        'soft judgment never sole veto',
        'MiniLM probabilities can differ across Node browser WASM WebGPU',
        'validate cutoffs in the deployment environment',
        'FP16 WebGPU needs a matching calibrated export',
        'same q8 weights are not the same probabilities',
        'BANKING77 TF-IDF 82.20% MiniLM 92.37% *theirs* not Harbor',
        'coverage 22.80% vs 82.23% *theirs*',
        'accepted accuracy 99.57% vs 97.95% *theirs*',
        'previously inspected test set',
        '75.0% (45/60) and 78.3% (47/60) *theirs*',
        'largest probability difference 0.278',
        'social unmeasured',
        'not evidence of reliable six-category routing',
        'FP16 98.66% at 77.75% coverage *theirs*',
        'not a new blind evaluation',
        'temperature 0.836752 threshold 0.90',
        'ECE 1.11% vs 0.83% *theirs*',
        '3 winning labels and 2 acceptance decisions',
        'browser guide still names jimothy-0.1.0.tgz',
        'Jev confidence field deliberately not reproduced',
        'wire-compat ≠ logit-equiv',
        'format version 3',
        'one question per bundle',
        'encoder fine-tuning not implemented in v0.1',
        'Xenova/all-MiniLM-L6-v2 q8',
        'checksums detect corruption not signatures',
        'catalog ≠ endorsement',
        'SHA move is not a replica',
        '*theirs* not Harbor',
        'Skip Archer',
        'invented_signal: false',
        'Parent merges only after ADV_PASS',
        'hourly 1851 §158 are on main',
        'this fold is §159 only',
        'composition 917-932',
        'findings batch #139',
        'do not reopen or amend PR #23 through #84',
        'does not bump 0.5.0 or 0.5.1',
        'notes.md §159',
    ):
        if frag not in UNIQ_JIMOTHY:
            failed.append(f"UNIQ_JIMOTHY missing {frag!r}")
    for rel in OVERLAYS:
        if rel == "research/notes.md":
            continue
        overlay_jimothy = (ROOT / rel).read_text(encoding="utf-8")
        if UNIQ_JIMOTHY in overlay_jimothy:
            failed.append(
                f"jimothy lock must stay in notes.md and the gate fixture, not {rel}"
            )
    ...
    if "## 161. Hourly 2044 HIGH" not in notes:
        failed.append("notes.md missing §161 heading")
    if notes.count(UNIQ_2044) != 1:
        failed.append(
            f"notes.md 2044 lock count {notes.count(UNIQ_2044)} != 1"
        )
    i161 = notes.find("## 161. Hourly 2044 HIGH")
    if not (i160 >= 0 and i161 > i160):
        failed.append("notes.md must stay contiguous §160 then §161")
    notes161 = notes[i161:] if i161 >= 0 else ""
    if "this fold is §161 only" not in notes161:
        failed.append("notes.md §161 missing §161-only occupancy")
    if "hourly 1949 §160 are on main" not in notes161:
        failed.append("notes.md §161 missing merged §160 occupancy")
    if UNIQ_2044 not in notes161:
        failed.append("2044 lock missing inside notes.md §161")
    for frag in (
        "TypeSafe Jev default path",
        ...
        "screen is a sensor",
        "no send path",
        "softmax over similarity is not a calibrated Noul",
        "skill mode is privileged simulator state",
        "Nemotron ≠ Archer",
        "temperature scaling is not RLCD",
        "Gemma 4 ≠ Archer",
        "trained Gemma 4 weights are not landed",
        "code owns the veto",
        "this card is not a trading procedure",
        "a gateway is not a decision model",
        "do not copy the proxy",
        "accuracy ≠ calibration",
        "routing ≠ permission",
        ...
        "invented_signal: false",
        "Parent merges only after ADV_PASS",
        "hourly 1949 §160 are on main",
        "this fold is §161 only",
        "composition 949-964",
        "findings batch #141",
        "do not reopen or amend PR #23 through #86",
        "does not bump 0.5.0 or 0.5.1",
        "notes.md §161",
        "openlayer-ai/jevals densify §126",
        "FluidInference/FluidUse densify §143",
        "danielyedaniel/jevme densify §156",
        "TMHSDigital/plumbline densify §158",
        "aiwithenoch/Jev-Skill densify §113",
        "thisisandreeeee/jev-benchmarks densify §114",
    ):
        if frag not in UNIQ_2044:
            failed.append(f"UNIQ_2044 missing {frag!r}")
    readme_2044 = (ROOT / "README.md").read_text(encoding="utf-8")
    if UNIQ_2044 in readme_2044:
        failed.append("README.md must not hold the 2044 uniqueness lock")
    for rel in OVERLAYS:
        if rel == "research/notes.md":
            continue
        overlay_2044 = (ROOT / rel).read_text(encoding="utf-8")
        if UNIQ_2044 in overlay_2044:
            failed.append(
                f"2044 lock must stay in notes.md and the gate fixture, not {rel}"
            )
    for card_id in (
"MiaoWuNYA/rikkahub-sillytavern-android",
"Iron-LYK/OmniJev",
"bokuweb/omg",
"1104480426-hash/jev-qq-assist",
"Roylaffman/jevGIS",
"Vankleben/jev-arm-lab",
"Waxmell114514/jev-trade",
"antTing/jev-accounts-hub",
"dhruvmehra/jevbench",
"hf:Cahol/laya-banking77-v1",
"xinyao27/jevonian",
"0x440-1me/laya-unity",
"ColinDargent/tri-emails-jev",
"Fate-Wen/jev-return-review-task",
"FuturePresentLabs/ooda",
"GuGuGuGun/battle_jev",
"GxZzzzz/WeChat_Galgame",
"HcodeQ/Langchain-Jev",
"IoTone/jevracket",
"Jaimejourney/mbti-jev-explorer",
"JohnRSandoval/meaning-assert",
"JohnRSandoval/meaning-aware-diff",
"MartinesEmanuel/athena-jev",
"NB3025/hello-jev",
"OhMyKing/JevTuringMachine",
"OpenNekoPaw/JevFlow",
"Thotheris/laya-gfx1030",
"WayneCommand/laya-mcp",
"ZongxingH/gemini-quality-gate-jev",
"abinashray008/fraud-classifier",
"andyjusa/laya-mlx-rs",
"anyran/jev-swot",
"bloodfel/modular-rag-mcp",
"boredrhino/Jevston",
"carolacodes/dspy_gliner_extraccion",
"dhirajpatra/agent-harness-with-jev-llm",
"dpvinh30092005/jev-1000-com-tam",
"dttfrancesco/lossless-rewrite",
"fruitflyworld/fruit-fly-world",
"garygentry/jev-poc",
"hf:ColomboAI/C3R-Decision-Laya-421M-v0.1",
"hf:KGFCode/laya-decision-lab",
"hf:liskasYR/laya",
"hf:liskasYR/laya-typed-decisions",
"hf:nampham1106/laya-flash",
"hgqimo/JevRanker",
"jackchen13755/dsh-jev-lens",
"jackma5477001/dsh-jev-auto",
"kingdsa/jev-chinese-grader",
"kuchris/laya-tetris",
"kydlikebtc/awesome-jev",
"lim6112j/jev-example",
"lzero07/laya-zh-eval",
"m4rtian/laya-it-triage",
"machaomc/jev-checkpoint",
"marceloatoledo/JEV-MCP-REST",
"mcmcmcmmmc/laya-codex-router",
"mmedasani1315/JEV-WebSiteClassifer",
"saivivekvenna/jevy-graph",
"scd13150/st-Mind-Compass",
"shivakrishna-k/jev-agent-routing-benchmark",
"slidwell44/silver-adventure",
"ttofalo/OlivIA",
"wanghai673/jev-browser-skill",
"wlawrencelo/Claude-code-Codex-Jev-harness",
"x007007007/laya-review",
"yangshun2005/laya-cn",
"yukihirop/jurl",
"zhazhahuiyuxiaoxiao/jev-codex-bench",
"openlayer-ai/jevals",
"FluidInference/FluidUse",
"danielyedaniel/jevme",
"TMHSDigital/plumbline",
"aiwithenoch/Jev-Skill",
"thisisandreeeee/jev-benchmarks"
    ):
        if card_id not in notes161:
            failed.append(f"notes.md §161 missing card {card_id}")

    if "## 162. Hourly 2149 HIGH" not in notes:
        failed.append("notes.md missing §162 heading")
    if notes.count(UNIQ_2149) != 1:
        failed.append(
            f"notes.md 2149 lock count {notes.count(UNIQ_2149)} != 1"
        )
    i162 = notes.find("## 162. Hourly 2149 HIGH")
    if not (i161 >= 0 and i162 > i161):
        failed.append("notes.md must stay contiguous §161 then §162")
    notes162 = notes[i162:] if i162 >= 0 else ""
    if "this fold is §162 only" not in notes162:
        failed.append("notes.md §162 missing §162-only occupancy")
    if "hourly 2044 §161 are on main" not in notes162:
        failed.append("notes.md §162 missing merged §161 occupancy")
    if UNIQ_2149 not in notes162:
        failed.append("2149 lock missing inside notes.md §162")
    for frag in (
        'TypeSafe Jev default path',
        'score the signal before the expensive call',
        'errors and provider failures stay eligible',
        'a scorer never writes',
        'the transcript never contained',
        '0.35 is their demo default',
        'Noul has no confidence field',
        'effective_confidence is a server formula',
        '0.5 is not a decision boundary',
        'the screen observes and does not send',
        'a relationship card is not a measurement of a person',
        'a nudge is not a merge gate',
        'fail-open when Jev errors',
        'a badge is not a takedown',
        'intent is not edit distance',
        'a suggestion is not authorization to run',
        'advisory routing never blocks',
        'routing ≠ permission',
        'a footer score is not a gate',
        'tests are not in the repo',
        'semantic channel BLOCKED',
        'any accuracy reading is not interpretable',
        'catalog ≠ endorsement',
        'description rewrite is not a new bench',
        'softmax over options is not a calibrated Noul',
        'Qwen3.5 ≠ Archer',
        'Qwen3-VL ≠ Archer',
        'a development panel is not a test set',
        'Frozen80 is not an independent test set',
        '71/80 68/80 66/80 *theirs*',
        'do not paste the prior Jev API 82.5% onto the 4B row',
        'empty SHA HTTP 409',
        'README HTTP 404',
        'Soft judgment never sole veto',
        'SHA move is not a replica',
        ...
        'invented_signal: false',
        'Parent merges only after ADV_PASS',
        'hourly 2044 §161 are on main',
        'this fold is §162 only',
        'composition 965-980',
        'findings batch #142',
        'do not reopen or amend PR #23 through #87',
        'does not bump 0.5.0 or 0.5.1',
        'notes.md §162',
        'hf:henrybit/jev-vs-deepseek densify §160',
        'kydlikebtc/awesome-jev densify §161',
        'loongWoong/jev-zen first card under a revisit tag',
        'Songokou1983/jev-mcp ≠ wangkuangkuang/jev-mcp-server',
        'zhongpei/jev-chat-jarvis ≠ jev-chat/jev-chat-jarvis'
    ):
        if frag not in UNIQ_2149:
            failed.append(f"UNIQ_2149 missing {frag!r}")
    readme_2149 = (ROOT / "README.md").read_text(encoding="utf-8")
    if UNIQ_2149 in readme_2149:
        failed.append("README.md must not hold the 2149 uniqueness lock")
    for rel in OVERLAYS:
        if rel == "research/notes.md":
            continue
        overlay_2149 = (ROOT / rel).read_text(encoding="utf-8")
        if UNIQ_2149 in overlay_2149:
            failed.append(
                f"2149 lock must stay in notes.md and the gate fixture, not {rel}"
            )
    for card_id in (
"reachjalil/jevlogs",
...
"202wh/loving-artist-practice",
"CommandCodeAI/cmd-mod-jev-nudge",
"jxxfdgd/jev-ai-slop-detector",
"marszhongx/pi-jev-score",
"syumai/jevyoumean",
"wellkilo/codex-jev-preflight",
"AbsoluteGeist/code-geist",
"AnakinRM/jev-confidence-kit",
"ArshanKhanifar/jevussy",
"Battleplus/jev_research",
"BubbleCal/vjev-serve",
"Daryl9441/laya-learning-demo",
"Eng-Ahmd/laya-breakout",
"ItisNoMatter/kojev",
"JackLee992/jev-reflex-automation",
"KO6BXL/jev-bot",
"KalyanM45/GitHub-Issue-Classification-Using-Jev",
"Learner-Lee/JEV-Try",
"Maxwell00000086/laya-agent-kit",
"NakliTechie/llamacpp-jev",
"R055LE/jev-plays-pokemon",
"Runtime-weekly/runtime-tutorials",
"Sorenskr/laya-player-generator",
"Suke-go/cognitive-transformation",
"actions-marketplace-validations/dtduc-git_jevnav",
"angribot/pi-jev",
"chepyle/jev-test",
"claudialu0720/jevaiworks",
"coderhh/jev-snake",
"davidkotler/jev-browser-tests",
"dgyun-ai/openclaw-typesafe-catalog-router",
"dpy22/go-jev",
"firede/laya-todo",
"furedea/reflex-state",
"gordan-code/dsh-jev-advisor",
"hf:Praveenrajus/jevify-qwen3-vl-2b-t2",
"hf:WIlfLin/JEV-Qwen3.5-0.8B-Base",
"hf:WIlfLin/JEV-Qwen3.5-9B-Base-NVFP4",
"hf:apus-ailab/APUS-OpenJev-Eval-Frozen80",
"hf:apus-ailab/APUS-OpenJev-v1-35B-A3B",
"hf:apus-ailab/APUS-OpenJev-v1-4B",
"hf:apus-ailab/APUS-OpenJev-v1-9B",
"hf:litert-community/Laya-English-LiteRT",
"hf:top7777/jev-schema-scorer-deberta-v3-large",
"humengna/jev_message",
"inhabitants/laya-snake-cuda",
"kairugakuo2/jev-arena",
"kelinbruce/jev-aico",
"layangdigitalinnovation/website-layang-digital",
"lesleygyc-ops/jev-connector",
"lim6112j/laya-example",
"llaplace-dev/jev-overcome-cartpole",
"ma2saka/jevmock01",
"manhua-man/jev-pilot-reflex",
"marcusglowe/jev-sort",
"nft-syou/jevcraft-bench",
"panchambanerjee/jev_expts",
"pgarvie/laya",
"philosophyAIEDU/260921jev",
"poiuyjie/jev_project_context",
"pythagoras-yamamoto/jev-qda",
"siren2345/jev-apple-coreml",
"tydong1231-ux/JevSteer",
"wangkuangkuang/jev-mcp-server",
"willyclayton/Jev-testing",
"youniszhang/jev-local-console",
"zceeeeee/jev-web-decesion-test",
"zhazhahuiyuxiaoxiao/jev-personal-radar",
"zhongpei/jev-chat-jarvis",
"hf:henrybit/jev-vs-deepseek",
"kydlikebtc/awesome-jev",
"loongWoong/jev-zen"
    ):
    ...
    if "885. **hosted Jev picks the click, code verifies**" not in algebra:
        failed.append("composition item 885 is not the 1751 click verification")
    if "901. **hosted MLX von port is not the PyTorch von**" not in algebra:
    ...
    if "949. **memory sieve is not the writer**:" not in algebra:
    ...
    if UNIQ_1751 in findings:
        failed.append("findings.md holds the 1751 uniqueness lock")
    if UNIQ_1851 in findings:
    ...
    if UNIQ_ONESYSTEM in readme:
        failed.append("README.md holds the one-system uniqueness lock")
    ...
    print(
        ...
        f"onesystem chars={len(UNIQ_ONESYSTEM)} "
