...
| Capability kernel / human-confirmed gate | Secrets never in the agent; closed action space; Jev SENSOR; policy BLOCK/ASK/ALLOW. Distinct from pre-exec toolgate. Human is the only kill trigger; identity re-check; shields override; mapped explanations not raw model prose. **Permission vs probability:** operator owns auto-approve thresholds; plugin never self-tunes the safety bar; host deny stays above (omp-greenlight; not a sandbox). **Privilege ≠ verdict:** effect-based shell gate; fast structural rules then Jev Choice + independent risk Nouls; fail-closed (construct-auto-classifier; 0 dangerous / 975 *theirs*) | `references/applied-mappings.md#7-capability-kernel--human-confirmed-gate` |
