...
| Skill / tool routing | Choice over a closed catalog + whether-anything-fits; code dispatches. Route ≠ memory: cheap intent gate skips memory tours on easy routes. Session-sticky first-prompt classification (lock for the session; fail-closed to a declared fallback). OMP/pi: `jev_route` topology/tier + `jev_acceptance_gate` before done (**fail-open**; contrast pi-jev-approver fail-closed) | `references/applied-mappings.md#5-skill--tool-routing` |
...
| Capability kernel / human-confirmed gate | Secrets never in the agent; closed action space; Jev SENSOR; policy BLOCK/ASK/ALLOW. Distinct from pre-exec toolgate. Human is the only kill trigger; identity re-check; shields override; mapped explanations not raw model prose | `references/applied-mappings.md#7-capability-kernel--human-confirmed-gate` |
...
| Org / safety control structure | Sensor ≠ constraint (Leveson); STPA if the sensor lies. Capability kernel: Jev SENSOR, policy.py constraint; secrets never in agent (interlock) | `references/mappings.md#8-control-structure--sensor--constraint-leveson` |
