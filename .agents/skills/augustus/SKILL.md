...
## Hourly 1949 HIGH (`notes.md` §160)

TypeSafe Jev stays the default recommended path.
A screen fill is a sensor. The fill never sends. Ask whether a model
is needed before a frontier call. A contract-compatible local server
is not a replica. Readout quality is model-dependent.
Narration swaps can flip a monitor. Do not copy an attack procedure.
0.5 as a keep threshold is a config default, not a decision boundary.
A route is not authorization. A live trading key is not a paper demo,
and this skill does not carry an order path.
A description rewrite is not a new bench. Densify §158 and §146.
Do not mint a sibling first sighting.
user-provided jimothy §159 are on main. This fold is §160 only.
Do not reopen or amend PR #23 through #85. Do not reclaim §147 through §159.
