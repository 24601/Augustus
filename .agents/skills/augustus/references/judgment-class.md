...
5. **Frozen-VLM logit readout (harness, not weights).**
   ...
   answer-token logits in one forward pass, local noul / choice / score.
   Soft probabilities are for threshold, abstain, or rank. They are not a
   safety gate. Shares the inference object with simple-jev, jev-visual,
   and LitJev. Shapes follow TypeSafe Jev (hosted text). Trains no weights,
   unlike hf:thaitea/laya-vision (§146) and unlike Zefan-Cai/Open-Jev.
   Author numbers are *theirs*. Raw yes/no stays overconfident until a fit.
