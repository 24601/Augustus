...
| Computer-use observe → score → act | Which observed control matches the current requirement | Score / Choice / option-attention among a11y/DOM candidates (Jev *or* GLiNER2 *or* Laya *or* Cua-S1); code clicks | Observation, freshness, dates; never generate selectors or values; independent outcome check (`DONE` ≠ success); plan ≠ execute | **Empirical recipe** as shipped loops (jev-ultrafast / solari-reflex Jev; gliner2-ultrafast GLiNER2; laya-mind2web DOM-index Laya); **Empirical as README/MODEL_CARD** for Cua-S1 (source-only, not TypeSafe Jev, `notes.md` §54); contrast blackwood-rlcd screenshot, `notes.md` §48, §52 |
...
| Cascade / prefilter (IR) | Cheap reject before an expensive scorer or LLM | Per-candidate Noul/Score; fail-open on drop, fail-closed on dispatch. Decision-native: evidence-set after wide retrieve | Candidate generation, always-keep set, recall keys; conflict/provenance | **Empirical recipe** (classifying RAG passages; jevprune; git-jev-stage). **Empirical as architecture** (decision-native-rag-skills; Hypothesis as a measured win, `notes.md` §55) |
...
| AST ∩ semantic lint | Semantic remainder after a parser already extracted units | Typed questions on Tree-sitter targets; do not execute scanned code | Parser, selection, fail-on; `tenbin` owns the lint *skill* | **Empirical as a shape** (jevscan 0.2.0rc4; not a calibration claim; `notes.md` §48) |
