...
| Routing / dispatch (OR) | Which queue/agent owns this item | Choice + confidence-gated escalation; code owns capacity | Cost matrix, capacity constraints | **Empirical recipe** (intent-routing; LlamaIndex Jev selectors; skillranker). **Empirical as README** skillranker VOI / abstention / hook fail-open (`notes.md` §66) |
| Named decision combinators | Then / Gate / Vote / Cascade / Weighted | Compose judgments as a circuit; trace the graph | AND/OR aggregation in code; fail polarity per act | **Empirical as README architecture** (decision-combinators; not literal Boolean AND/OR; `notes.md` §66) |
...
| Evidence-gated question pack | Is this criteria set measured on a pinned version | Record accuracy/ECE/cost/latency; `unknown` on every Choice/Score | Pin model; no numbers → not `verified` | **Empirical as registry tables** (jev-packs; nine packs *theirs*; jevassert 404/unreleased; `notes.md` §64). **Hunch:** measurement owns endorsement |
