...
| Computer-use observe → score → act | Which observed control matches the current requirement | Score / Choice among a11y/DOM candidates (Jev *or* GLiNER2 *or* Laya); code clicks | Observation, freshness, dates; never generate selectors; independent outcome check (`DONE` ≠ success) | **Empirical recipe** (jev-ultrafast / solari-reflex Jev; gliner2-ultrafast GLiNER2; laya-mind2web DOM-index Laya; contrast blackwood-rlcd screenshot, `notes.md` §48, §52) |
