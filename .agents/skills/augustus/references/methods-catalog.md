...
| Cascade / prefilter (IR) | Cheap reject before an expensive scorer or LLM | Per-candidate Noul/Score; fail-open on drop, fail-closed on dispatch. Decision-native: evidence-set after wide retrieve. Classify-first MCP: content to the judge without entering main agent context first | Candidate generation, always-keep set, recall keys; conflict/provenance; hard I/O envelope | **Empirical recipe** (classifying RAG passages; jevprune; git-jev-stage). **Empirical as architecture** (decision-native-rag-skills; Hypothesis as a measured win, `notes.md` §55). **Empirical as README** (jev-sift; mocks ≠ accuracy, `notes.md` §56) |
