...
## Apply 0923 (`notes.md` §144)

Same pillars, hourly HIGH (intellyweave GLiNER OSINT, openvons finite-choice
softmax, AgentBeam local policy, unofficial Go SDK, JevPokerBench virtual
chips, patdown/evoke/prompt2jev densify; AI / SWE / business / knowledge
work, not SWE-only). Do **not** re-fold §143 0823 / §142 0707 / §121
patdown / §139 evoke / §141 prompt2jev as a second census. Densify prior
cards in place.
Fresh PR off `89a26925295e` (merged #67). Never reopen merged #7–**#67**.
Skip Archer rewrite (still **promised_not_landed**). Quote *theirs*.
Soft Noul ≠ hard safety. Formal methods **compose** with scoring; a
Noul is a SENSOR. Treating GLiNER locate as decide, openvons 0.916 as
Harbor, poker chips as calibrated Noul, or a retired name reservation
as a replica is the anti-pattern.

1. **GLiNER OSINT / Locate ≠ decide.**
   intellyweave 76★ BSD-3-Clause. Locate ≠ decide.
...
3. **AgentBeam local security layer / soft scores ≠ hard gates.**
   unofficial not affiliated. wire-compat ≠ logit-equiv. chips virtual.
   game success ≠ calibrated Noul.
4. **densify §121 / §139 / §141 not sibling first sightings.**
   Block/steer/fuzzy lint. Jev is the first adapter; the design is bound to no engine.
   heuristic conversion ≠ calibrated Noul.
5. **namesakes / skip-thin gnapse/jev-cli HTTP 404 / retired name reservation is not a replica.**
...
Soft Noul ≠ hard safety: 0.916 / 22.6 ms / Median 275 ms / trolley 0.99 are **sensors**.
SHA move is not a replica. Do not reopen or amend PR #23–#67.

## Apply 0823 (`notes.md` §143)

Same pillars, hourly HIGH (dohnuts MODEL_CARD densify, FluidUse on-device
form CU, third-hand screenshots stay local, mini-jev letter logits,
jev-benchmarks *theirs*; AI / SWE / business / knowledge work, not
SWE-only). Do **not** re-fold §142 0707 / §141 0551 / §137 dohnuts as a
second census. Densify prior cards in place.
Fresh PR off `41fa40c` (merged #66). Never reopen merged #7–**#66**.
Skip Archer rewrite (still **promised_not_landed**). Quote *theirs*.
Soft Noul ≠ hard safety. Formal methods **compose** with scoring; a
Noul is a SENSOR. Treating JevBench 65.80% as Harbor, same-species
serving as an 18th scoring row, field→value as free text, or Cua-S1 as
TypeSafe is the anti-pattern.

1. **JevBench 65.80% vs Jev 86.58% / Laya multi 47.62% *theirs* /
   same-species serving not an 18th scoring row.**
   densify §137 not a sibling first sighting. Dohnuts ≠ TypeSafe.
2. **field→value match among supplied options not free text /
   Cua-S1 ≠ TypeSafe.**
   Accessibility API. CUA-S1-FORMS CoreML ~706K params ~1ms Neural Engine.
3. **Screenshots aren't uploaded / Jev is the only model / not fully offline.**
   JSON 0.909 letters 0.907 *theirs*. 13 600 / 13 600 *theirs*.
   softmax over letters ≠ calibrated Noul.
4. **AG News 0.910 *theirs* / Fastest and cheapest web agent *theirs*.**
   model=jev-auto. routing ≠ permission. densify description rewrite.
   densify §142 not a sibling first sighting.
5. **namesakes / skip-thin GokhanCalkap/LayaCode fredzhaozonghui/LAYA1 empty SHA.**
...
Soft Noul ≠ hard safety: 65.80% / 0.910 / 0.909 are **sensors**.
SHA move is not a replica. Do not reopen or amend PR #23–#66.

## Apply 0707 (`notes.md` §142)

Same pillars, hourly HIGH (Jev-cu text-only CU, tax-doc corpus *theirs*,
mario without screenshots, mobile-jev booking not demonstrated,
AbdelStark densify §141, ENEM 2025 *theirs*; AI / SWE / business /
knowledge work, not SWE-only). Do **not** re-fold §141 0551 / §140 0445
as a second census. First-party 0551 SHA bumps are not densify REVISIT.
Fresh PR off `34edba4` (merged #65). Never reopen merged #7–**#65**.
Skip Archer rewrite (still **promised_not_landed**). Quote *theirs*.
Soft Noul ≠ hard safety. Formal methods **compose** with scoring; a
Noul is a SENSOR. Treating text-only CU as screenshots, 100% of a
corpus as Harbor, Mario as Noul, or a completed booking as demonstrated
is the anti-pattern.
...
3. **The model does not receive screenshots / game success ≠ calibrated Noul.**
   21 seconds for 9 actions *theirs*. A completed booking is not demonstrated.
4. **The field guide to typed decisions / densify §141 not a sibling first sighting.**
   hf:Praveenrajus/jev-bench HTTP 401 was 200. densify §107/§125 remainder.
   densify §134 not a sibling first sighting. ENEM 2025 *theirs* not Harbor.
5. **namesakes / skip-thin developerekene/System-One holotwist/laya wuzhiping/jev-laya empty SHA.**
   droidrun/mobile-jev ≠ Friedjof/jev-mobile.
   itsmostafa/typesafe-mcp ≠ cyrusasco/typesafe-mcp ≠ burnigtm/jev-mcp.

Soft Noul ≠ hard safety: 100% of corpus / 21s / ENEM 56.6% are **sensors**.
SHA move is not a replica. Do not reopen or amend PR #23–#65.

## Apply 0551 (`notes.md` §141)

Same pillars, hourly HIGH (kev Night-2 densify plus Nimble holdout
publish, APUS Frozen80 *theirs*, Jev-Vision Hub LoRA, evoke adapter
rewrite, GLiNER Locate ≠ decide; AI / SWE / business / knowledge work,
not SWE-only). Do **not** re-fold §140 0445 / §139 0348 / §45 kev /
§35 Nimble as a second census.
Fresh PR off `524c2d2` (merged #64). Never reopen merged #7–**#64**.
Skip Archer rewrite (still **promised_not_landed**). Quote *theirs*.
Soft Noul ≠ hard safety. Formal methods **compose** with scoring; a
Noul is a SENSOR. Treating locked OOD 0.852 as Harbor, Frozen80 85.0%
as TypeSafe, Locate as decide, or a catalog as endorsement is the
anti-pattern.

1. **Night-2 sign-off / densify §45 not a sibling first sighting.**
   locked OOD 0.684/0.837/0.852 *theirs*. temperature scaling ≠ ECE
   unless measured. Hub --revision is a pin not a replica.
...
3. **APUS-OpenJev 9B 85.0% vs Jev API 82.5% *theirs* not Harbor /
   Candidate probabilities are not calibrated confidence.** Frozen80
   n=80. JevBench hard 52% at 91% mean confidence *theirs*.
4. **Jev is the first adapter the design is bound to no engine /
   Zero Hallucinations marketing / Locate ≠ decide.** densify §139
   not a sibling first sighting. catalog ≠ endorsement.
5. **namesakes / skip-thin eatmoreduck/jev-jarvis.**
...
Soft Noul ≠ hard safety: 0.852 / 85.0% / 90.1% / 98.6% are **sensors**.
SHA move is not a replica. Do not reopen or amend PR #23–#64.

## Apply 0445 (`notes.md` §140)

Same pillars, hourly HIGH (lcc real Laya keep-all plus Jev-Compatible
softmax gateway, any2jev ECE 0.027 *theirs*, NanoJev JevHarness densify
§115, How you ask mattered more, Calibration is not yet measured; AI /
SWE / business / knowledge work, not SWE-only). Do **not** re-fold
§139 0348 / §138 0248 / §115 NanoJev as a second census.
Fresh PR off `777546f` (merged #63). Never reopen merged #7–**#63**.
Skip Archer rewrite (still **promised_not_landed**). Quote *theirs*.
Soft Noul ≠ hard safety. Formal methods **compose** with scoring; a
Noul is a SENSOR. Treating lcc keep-all as cost reduction, 232.1 ms as
logit-equiv, ECE 0.027 as Harbor, or arcade 70/80 as calibrated Noul is
the anti-pattern.

1. **lcc keep-all / Token reduction alone is not cost reduction.**
   keeps essentially every block 0.0%/−0.5% *theirs*. mock Laya = Jev
   −22.6% on XL withdrawn. N=18 pilot not Harbor.
2. **Chat 1282.3 ms vs gateway 232.1 ms ~1/5.5 *theirs* / Softmax over candidate logprobs.**
   Qwen3.8-27B ≠ Archer. wire-compat ≠ logit-equiv.
3. **acc 0.796 ECE 0.027 *theirs* / How you ask mattered more /
   Calibration is not yet measured.** three seeds not Harbor. game
   success ≠ calibrated Noul.
...
are **sensors**.
SHA move is not a replica. Do not reopen or amend PR #23–#63.

## Apply 0348 (`notes.md` §139)

Same pillars, hourly HIGH (open-cricket BYOM Qwen2.5 plus jev-decision-arena
confidence ≠ P(correct), typesafe-ai-test 8,400 calls, werr JevBench 81.65
*theirs*, OpenJev-Kit training not complete, jev-hooks / jury.nvim / evoke,
voice-browser densify §82; AI / SWE / business / knowledge work, not
SWE-only). Do **not** re-fold §138 0248 / §137 0151 / §82 voice-browser
as a second census.
Fresh PR off `04c8a44` (merged #62). Never reopen merged #7–**#62**.
Skip Archer rewrite (still **promised_not_landed**). Quote *theirs*.
Soft Noul ≠ hard safety. Formal methods **compose** with scoring; a
Noul is a SENSOR. Treating open-cricket as TypeSafe, Greedy 0.90 as
P(correct), JevBench 81.65 as Harbor, or a clean scanner report as a
sandbox is the anti-pattern.

1. **open-cricket BYOM / wire-compat ≠ logit-equiv.** Qwen2.5 ≠ Archer.
   replica ≠ TypeSafe. API follows Jev's general call shapes but model
   predictions and confidence calibration differ.
2. **Greedy 0.90 vs Oracle 0.82 *theirs* / confidence ≠ P(correct).**
   Random conf 0.00 still 20.5% *theirs*. seed 42 n=1 is not Harbor.
...
4. **Compose meaning like state / densify §82 / A clean report is not proof.**
   context is the conversation so far. does not sandbox.
   generation from Choice is not a language model replica.
   catalog ≠ endorsement.

Soft Noul ≠ hard safety: Greedy 0.90 / Noul 0.7 / JevBench 81.65 / WindTunnel
49/49 are **sensors**.
SHA move is not a replica. Do not reopen or amend PR #23–#62.

## Apply 0248 (`notes.md` §138)

Same pillars, hourly HIGH (Knowledgator GLiClass Hub class-peer catalog
plus typed-decision leaderboard *theirs*, Jevbridge ACP/MCP adapter,
Sharp / playground / Aside first cards, serving GGUF/MLX/LiteRT; AI /
SWE / business / knowledge work, not SWE-only). Do **not** re-fold
§137 0151 / §136 0049 / §123 GLiClass ports as a second census.
Fresh PR off `5b56b54` (merged #61). Never reopen merged #7–**#61**.
Skip Archer rewrite (still **promised_not_landed**). Quote *theirs*.
Soft Noul ≠ hard safety. Formal methods **compose** with scoring; a
Noul is a SENSOR. Treating Hub GLiClass as Jev, 0.7350 as Harbor,
Jevbridge as logit-equiv, or LiteRT as calibrated Noul is the
anti-pattern.

1. **GLiClass class-peer catalog not Jev equivalent.** instruct-large PRIMARY.
   Knowledgator/GLiClass.c already §123. Hub models first card as class-peer entries.
2. **typed-decision-leaderboard *theirs* not Harbor / wire-compat ≠ logit-equiv.**
   JEV 0.7350 ZTC 27B 0.7289 *theirs*. Jevbridge ACP and MCP adapter.
3. **Cut the slop / Not a Cua binding / open reproductions of the shape.**
   Sharp. Aside. nico-martin/open-jev ≠ razorback16/openjev.
4. **serving substrate ≠ calibrated replica / already carded / skip-thin.**
   82.3% ECE 0.017 *theirs*. catalog ≠ endorsement.

Soft Noul ≠ hard safety: 0.7350 / 82.3% / 91.33% / 144/144 are **sensors**.
SHA move is not a replica. Do not reopen or amend PR #23–#61.

## Apply 0151 (`notes.md` §137)

Same pillars, hourly HIGH (Open-Jev v3 community data + held-out + site
densify plus jev-wide 83%, certo, first-instinct, JevOps gate, dohnuts,
Chrome community port; AI / SWE / business / knowledge work, not
SWE-only). Do **not** re-fold §136 0049 / §135 2347 / §125 Open-Jev
densify as a second census.
Fresh PR off `9b0bc2c` (merged #60). Never reopen merged #7–**#60**.
Skip Archer rewrite (still **promised_not_landed**). Quote *theirs*.
Soft Noul ≠ hard safety. Formal methods **compose** with scoring; a
Noul is a SENSOR. Treating v3 rows as a released replica, a held-out
protocol as Harbor, 83% as Harbor, or Jev as a Lean writer is the
anti-pattern.

1. **v3 data prepared ≠ retrained released models / held-out protocol ≠ Harbor.**
   Open-Jev densify PRIMARY. 129,288 / 74,921. 1,280-row panel ≠ Harbor.
   finite training loss ≠ quality improvement. website redesign ≠ calibration.
2. **naive throws away 83% *theirs* / certo KL 0.008 / first-instinct 63.3%→78.1%.**
   *theirs* not Harbor. 371,278 prepared ≠ consumed.
3. **Jev is a gate not a generator / community port ≠ TypeSafe.**
   Lake remains admission. Jev never writes Lean. Dohnuts ≠ TypeSafe.
4. **held-out before winner / namesakes / skip-thin.**
   demo accuracy is synthetic not a Jev benchmark. catalog ≠ endorsement.

Soft Noul ≠ hard safety: 83% / KL 0.008 / 63.3%→78.1% / 1,280-row are **sensors**.
SHA move is not a replica. Do not reopen or amend PR #23–#60.

## Apply 0049 (`notes.md` §136)

Same pillars, hourly HIGH (Open-Jev JevBench public-subset densify plus kev
35B MMLU-Pro, browser-use 5-10×, fail-open and ordered routers, Gemma open
heads; AI / SWE / business / knowledge work, not SWE-only). Do **not**
re-fold §135 2347 / §134 2246 / §125 Open-Jev densify as a second census.
Fresh PR off `28d5c81` (merged #59). Never reopen merged #7–**#59**.
Skip Archer rewrite (still **promised_not_landed**). Quote *theirs*.
Soft Noul ≠ hard safety. Formal methods **compose** with scoring; a
Noul is a SENSOR. Treating 231 as 534, 0.550 as Harbor, 5-10× as a
replica, or fail-open routing as a grant is the anti-pattern.

1. **public-subset ≠ Harbor / 231 ≠ 534.** Open-Jev densify PRIMARY.
   Publish audited JevBench public-subset baselines. 231 public tasks.
   do not report full-534. Open-Jev TREC pending.
2. **PLAN correct 35B MMLU-Pro (0.550) / evaluate.load honour weights_dtype=bf16.**
   kev densify. Not shipped. Qwen3.6 ≠ Archer.
3. **5-10× *theirs* not Harbor / fail-open routing ≠ permission.**
   jev-browser-use. gargpratyush/jev-router. ordered routing ≠ end-to-end.
4. **softmax next-token ≠ calibrated Noul / potential_match ≠ hiring decision.**
   daseinlabs/open-jev. system-one-gemma 64.4% ECE 0.047 *theirs*.
   catalog ≠ endorsement.

Soft Noul ≠ hard safety: 64.94% / 0.550 / 5-10× / 38% 44% are **sensors**.
SHA move is not a replica. Do not reopen or amend PR #23–#59.

## Apply 2347 (`notes.md` §135)

Same pillars, hourly HIGH (openjev MLX serving-port densify plus TypeLLM
Release v0.1.1, JevLoop independent loop, option-order measurement,
tokenizer, TomatoEggBench, n=8 pilot, compaction ranking; AI / SWE /
business / knowledge work, not SWE-only). Do **not** re-fold §134 2246
/ §133 2146 / §75 openjev densify as a second census.
Fresh PR off `427dcf4` (merged #58). Never reopen merged #7–**#58**.
Skip Archer rewrite (still **promised_not_landed**). Quote *theirs*.
Soft Noul ≠ hard safety. Formal methods **compose** with scoring; a
Noul is a SENSOR. Treating MLX text gen as a calibrated Noul, a GitHub
Release as calibration, n=8 as Harbor, or Cua-S1 as TypeSafe Jev is the
anti-pattern.

1. **dual serving is not generate.** openjev MLX densify PRIMARY.
   MLX backend steps>1/think/text gen + image Qs. Hosted Codiv ≠ TypeSafe.
   wire-compat ≠ logit-equiv.
2. **GitHub Release v0.1.1 / Constrained AR ≠ calibrated Noul.**
   TypeLLM densify. README SHA unchanged. PyPI packaging ≠ calibrated Noul.
3. **option order can change an answer / n=8 is not Harbor.**
   0.188 or 0.542 *theirs*. jevtok 0 mismatches *theirs* not Harbor.
   ockev 35ms 95.8% TomatoEggBench *theirs* not Harbor.
4. **does not execute / catalog ≠ endorsement.** JevLoop independent
   not affiliated. ranking before lossless condensation. Cua-S1 ≠ TypeSafe.

Soft Noul ≠ hard safety: 74.6% / 95.8% / 0 mismatches / n=8 are **sensors**.
SHA move is not a replica. Do not reopen or amend PR #23–#58.

## Apply 2246 (`notes.md` §134)

Same pillars, hourly HIGH (Open-Jev TREC prep densify plus TypeLLM PyPI,
simple-jev first card, directories, memory controller, abandoned LoRA
retrospective, Go/Rust serving, HF packs; AI / SWE / business /
knowledge work, not SWE-only). Do **not** re-fold §133 2146 / §132 2049
/ §125 Open-Jev densify as a second census.
Fresh PR off `f635ffe` (merged #57). Never reopen merged #7–**#57**.
Skip Archer rewrite (still **promised_not_landed**). Quote *theirs*.
Soft Noul ≠ hard safety. Formal methods **compose** with scoring; a
Noul is a SENSOR. Treating a CPU-passing TREC prep as nDCG, a PyPI
wheel as a calibrated Noul, next-token logits as P(correct), or a
directory as endorsement is the anti-pattern.

1. **TREC prep ≠ completed Open-Jev TREC.** Open-Jev densify PRIMARY.
   Actual Open-Jev TREC model inference is pending. All 79 combined
   CPU tests pass. context proof ≠ nDCG. CPU tests ≠ GPU scores.
2. **PyPI packaging ≠ calibrated Noul.** TypeLLM densify. typellm 0.1.1.
   Constrained AR ≠ calibrated Noul.
3. **logits are not calibrated probabilities of correctness.**
   simple-jev 408★. wire-compat ≠ logit-equiv. does not reproduce TypeSafe.
4. **catalog ≠ endorsement.** jev-directory 50 evals / 1300+ builds.
   Jev-Mem 11.0% 6.6× 36.7% *theirs* not Harbor.

Soft Noul ≠ hard safety: 79 CPU tests / 11.0% / 408★ are **sensors**.
SHA move is not a replica. Do not reopen or amend PR #23–#57.

## Apply 2146 (`notes.md` §133)

Same pillars, hourly HIGH (Open-Jev provider quality densify plus cartpole
Kev flip, ashare rewrite, kevin first card, and novel class members; AI /
SWE / business / knowledge work, not SWE-only). Do **not** re-fold §132
2049 / §131 1946 / §125 Open-Jev densify as a second census.
Fresh PR off `36b26ba` (merged #56). Never reopen merged #7–**#56**.
Skip Archer rewrite (still **promised_not_landed**). Quote *theirs*.
Soft Noul ≠ hard safety. Formal methods **compose** with scoring; a
Noul is a SENSOR. Treating CPU tests as GPU scores, CartPole as hosted
Jev, QMT as live orders, 3.69ms as Harbor, 80.1% as gold, or 95% as a
hard gate is the anti-pattern.

1. **provider pipeline ≠ completed Open-Jev quality.** Open-Jev densify
   PRIMARY. 808 requests 1841 labelled. Open-Jev GPU inference has not
   started. 48 CPU tests pass. CPU tests ≠ GPU scores.
2. **fine-tuned Kev ≠ TypeSafe Jev.** cartpole densify. 81.25% 52/64
   *theirs*. one record of 64. softmax ≠ calibrated Noul.
3. **does not execute.** ashare densify. QMT mock/dry default no
   orders. AUC 0.532 *theirs*.
4. **cutoff 95% still soft / catalog ≠ endorsement.** vercel form
   router, metask 80.1% *theirs*, kevin 3.69ms *theirs* not Harbor,
   typedecide 87 of 144, yunhe-dev/awesomejev.

Soft Noul ≠ hard safety: 65/76 / 80.1% / 3.69ms / 78.24 are **sensors**.
SHA move is not a replica. Do not reopen or amend PR #23–#56.

## Apply 1946 (`notes.md` §131)

platform does not execute trades. catalog ≠ endorsement.
AI-reviewed labels ≠ gold. one-trial robot ≠ Harbor.
10.59× systems ≠ ECE. agreement ≠ accuracy. Qwen3.8 ≠ Archer.
Spanish −6.4 pp XNLI *theirs*. ECE 0.057→0.101 *theirs*.
72.2% vs 63.4% p_max≥0.9 coverage *theirs*.
desc rewrite ≠ SHA/behavior change. heuristic conversion ≠ calibrated Noul.
rule-table ≠ model. local_only ≠ Jev. replica ≠ TypeSafe.
