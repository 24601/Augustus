...
| Independent evidence catalog (not Harbor) | [reachjalil/system-one-bench](https://github.com/reachjalil/system-one-bench) | JavaScript MIT; **0★**; README SHA `3e7109bf`. independent System One evidence catalog. scores not one leaderboard. no external record currently reproduced. TokenTrim no-Jev matched hybrid 62.4%. ≠ mallahyari. `notes.md` §103 |
...
| ChatJev anti-pattern (no eval) | [erik-dunteman/ChatJev](https://github.com/erik-dunteman/ChatJev) | Python; license null; **0★**; README SHA `c763be19`. Jev classifier as autoregressive next-token predictor. ChatJev-style soundness theater. ≠ dannote/jev ≠ jev-gpt. `notes.md` §103 |
