...
| Hard-gate enzyme `when asked` / treat catalysts as summaries | Guidance ≠ hook; index ≠ summary | guidance ≠ hook; catalysts ≠ summaries; compile-time System One. `notes.md` §94 |
| Treat unofficial JA ModernBERT as TypeSafe / skip format_version | Unofficial; lock | unofficial ≠ TypeSafe; format_version modernbert-jev/1. Argos1111/jev_local ≠ us/jev-local ≠ kunchenguid/local-jev. `notes.md` §94 |
