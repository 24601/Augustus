...
| Collapse ywchiu into Running-Dolphins / treat 77.0% as Harbor | Harbor-jevals honesty | ywchiu/jev_benchmark ≠ Running-Dolphins/jev-bench ≠ Praveenrajus/jev-bench. None of the systems are particularly good at knowing when to stop and ask. They skip the question and call a tool directly. `notes.md` §112 |
| Collapse transformers into llama.cpp / softmax as Noul | Wire-compat ≠ replica | siren2345/jev-single-decode-transformers ≠ siren2345/jev-single-decode. Split Transformers experiment from llama.cpp runtime. `notes.md` §112 |
| Collapse tanay into dairui1 / BrendanH18 / yibie | Namesake | tanayvasishtha/jev-lab ≠ dairui1/jev-lab ≠ BrendanH18/jev-lab ≠ yibie/laya-jev-lab. The pages never call Jev. `notes.md` §112 |
| Treat TF-IDF ECE as beating Jev / 0.85 as 85% | Ranking ≠ calibration | Heman10x-NGU/Verdict-open-jev ≠ Heman10x-NGU/openJev-verdict-2.0. TF-IDF + LogReg ECE 0.0207 vs Jev 0.1440. 0.85 coverage 84.60% selective risk 1.18%. `notes.md` §112 |
| Collapse altryne into Mintzs / invent tweets | Namesake / no invented discourse | altryne/jevify ≠ Mintzs/jevify ≠ gulagala001/jevify ≠ uspraveen/Jevify. Find where Jev belongs. Design the questions. Measure the difference. `notes.md` §112 |
| Treat Brier-on-stated-conf as RLCD / distill Jev as teacher | Trap / teacher-copy | Finding 1: Brier on stated confidence alone is a trap. Student B MAE 0.148 / Pearson 0.836 / 86.0%. `notes.md` §112 |
| Treat ACT as a provider proof / collapse Jev-Skill into simplosophy | Policy ≠ provider | ACT / REVIEW / FALLBACK. A provider failure, timeout, malformed output, or missing answer is **not** a policy outcome. aiwithenoch/Jev-Skill ≠ simplosophy/jev-skill ≠ laguagu/jev-skills. current-llm. 结构兼容，不是 Jev 模型能力. `notes.md` §112 |
