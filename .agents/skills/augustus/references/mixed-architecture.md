...
| Treat a catalog row as a bake-off win / promote reported to reproduced / collapse into mallahyari | **Fail closed** (evidence catalog; not a leaderboard) | independent System One evidence catalog. scores not one leaderboard. no external record currently reproduced. TokenTrim no-Jev matched hybrid 62.4%. reachjalil/system-one-bench ≠ mallahyari/system-one-benchmark. `notes.md` §103 |
...
| Independent System One evidence catalog | 12 reviewed records; `reported`/`first-party`/`reproduced`/`reference` | scores not one leaderboard; no external record currently reproduced | [reachjalil/system-one-bench](https://github.com/reachjalil/system-one-bench) (JavaScript MIT; **0★**; HEAD `4a83bae6`; README SHA `3e7109bf`; GitHub size **0** with contents). independent System One evidence catalog. TokenTrim no-Jev matched hybrid 62.4%. reachjalil/system-one-bench ≠ mallahyari/system-one-benchmark. `notes.md` §103 |
...
| Frozen-LLM typed decisions | 93.25% self-authored *theirs*; residual-head hurt 73/96→67/96 | confidence = 1−normalized entropy, not P(correct) | [yuki-oshio/mini-jev](https://github.com/yuki-oshio/mini-jev) (Python MIT; **0★**; HEAD `dff5b323`; README SHA `363441b6`). frozen local LLM logits, no trained decision head. yuki-oshio/mini-jev ≠ r-ms/mini-jev. `notes.md` §103 |
| AR next-token anti-pattern | “kinda works”; pyproject name `jev` | ChatJev-style soundness theater; never free-generates is jev-gpt | [erik-dunteman/ChatJev](https://github.com/erik-dunteman/ChatJev) (Python; license null; **0★**; HEAD `ea33ab8d`; README SHA `c763be19`). Jev classifier as autoregressive next-token predictor. erik-dunteman/ChatJev ≠ dannote/jev ≠ jev-gpt. `notes.md` §103 |
...
| Parallel rank vs serial selection | serial max vs parallel rank-k; integers −1e6..1e6 | independent questions can conflict | [zzzzzec/jevsort](https://github.com/zzzzzec/jevsort) (HTML; license null; **1★**; HEAD `2d960ab9`; README SHA `cbe5e6bf`). parallel rank-prediction vs serial selection. zzzzzec/jevsort ≠ keltokhy/jsort. `notes.md` §103 |
| Open-side ecosystem catalog | models/evals/calibration/constrained-decoding | not closed-Jev apps; do not paste listed numbers | [rupeshpoojary9/awesome-open-system-one](https://github.com/rupeshpoojary9/awesome-open-system-one) (CC0 1.0; SPDX NOASSERTION; **0★**; HEAD `637ee3d3`; README SHA `0007e343`). curated open System One ecosystem catalog. rupeshpoojary9/awesome-open-system-one ≠ AnotiaWang/awesome-jev. `notes.md` §103 |
