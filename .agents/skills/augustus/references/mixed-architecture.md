...
| Extractive quotes / pointer evidence | Per-sentence, per-line-id, or char-offset Noul/Choice | Verbatim join; place; `redecide` / CSV; model never writes the excerpt | testimonial-miner; jev-reviewer; gliner25-compaction; **gliner-native-runtime** (Swift/Core ML spans + confidence; `notes.md` §97) |
