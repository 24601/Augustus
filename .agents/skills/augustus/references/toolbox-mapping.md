...
| Safety engineering: STPA | Sensor ≠ constraint; table of unsafe control actions if the sensor lies | **Contract** as ownership (`mappings.md` §8; Leveson). **Empirical as architecture** (interlock: Jev SENSOR, policy.py constraint, secrets never in agent; type-safe ≠ correct; `notes.md` §59) |
