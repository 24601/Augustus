...
| Experimental design: evidence-gated question packs | Pack.yaml + golden cases + evidence.md; pin version; no numbers → not verified | **Empirical as registry** (jev-packs nine packs *theirs*; jevassert unreleased; `notes.md` §64). **Hunch:** measurement owns endorsement |
...
| Experimental design: pointer compact vs LLM summarize | Saved tokens vs invented paths vs early-fact recall vs latency/cost | **Empirical as one synthetic session** (jev-compactor 64.5%/366ms/0 hallucinated/4 of 4 vs Sonnet 96.2%/1 invented path; `notes.md` §65) |
